...
class Rectangle : public Shape {
    ...
    bool intersection(const Ray d , Vector& P, Vector& N, double &t) {
        ...
        if(x1 <= tmp1 && tmp1 <= x2 && y1 <= tmp2 && tmp2 <= y2){
            double t = (z - d.origin[2]) / d.direction[2];
            P = d.origin + t*d.direction;
            Vector v(0, 0, 1);
            N = v;
            return true;
...
class Scene {
    public : 
        Scene() {};
        void addSphere(const Sphere& s) { shapes.push_back(std::unique_ptr<Shape>(new Sphere(s.O, s.R, s.albedo))); }
